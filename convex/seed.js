...
// ✅ Ensure minimum 3 users (DEV ONLY)
if (users.length < 3) {
  const needed = 3 - users.length;

  for (let i = 0; i < needed; i++) {
    await ctx.db.insert("users", {
      userId: `seed_user_${users.length + i + 1}`,
      name: `Seed User ${users.length + i + 1}`,
      email: `seed${users.length + i + 1}@test.com`,
    });
  }

  // re-fetch users after insert
